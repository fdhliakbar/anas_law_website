<template>
  <section class="py-16 bg-white text-gray-800">
    ...
    <div v-if="loading" class="flex justify-center items-center py-12">
      <div
        class="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#B49F2B]"
      ></div>
    ...
    >
      <div
        ...
        @click="goToExternalLink(post.link_artikel)"
      >
        <img
          :src="post.image"
          :alt="post.title"
        ...
        <div class="p-6">
          ...
          <div class="flex items-center gap-3 mt-auto">
            ...
            <div>
              ...
              <p class="text-xs text-gray-400" v-if="post.created_at">
...
const fetchArticles = async () => {
  ...
  try {
    const response = await fetch(
      "https://mptibe-production.up.railway.app/api/article/get-articles?limit=3&offset=0"
    );
...
const goToExternalLink = (url) => {
  if (url) {
    ...
    console.warn("Link artikel kosong");
  }
};
...
const blogPosts = computed(() => {
  ...
  return articles.value.map((article) => ({
    ...
    author: "Admin Anas Law",
    company: "Anas Law Firm",
    authorImage: "/src/assets/images/founder.jpg",
    artikel_id: article.artikel_id,
    created_at: article.created_at,
    link_artikel: article.link_artikel, // 🔥 ini penting
  }));
});
