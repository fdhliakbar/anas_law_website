    ...
    >
      ...
      <nav class="flex-1 px-4 py-6">
        ...
        <div class="mt-10">
          ...
          <ul class="space-y-1">
            <li>
              <button @click="goToLawyerManagement" class="w-full flex items-center text-sm text-gray-700 hover:text-indigo-600 hover:bg-indigo-50 px-2 py-1 rounded transition-colors duration-200">
    ...
    <div class="flex-1 flex flex-col md:ml-0">
      ...
      >
        <div class="flex items-center">
          ...
          <h1 class="text-2xl font-bold text-gray-800">Dashboard Admin - Anas Law</h1>
        </div>
      ...
      <main class="flex-1 p-8 overflow-y-auto">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div class="bg-white rounded-xl shadow p-6 flex flex-col">
            <div class="flex items-center justify-between mb-2">
              <span class="font-semibold text-gray-700">Artikel Dipublikasi</span>
              <span class="material-icons text-purple-400">article</span>
            </div>
            <div class="text-2xl font-bold text-purple-600">{{ stats.totalArticles }}</div>
            <div class="text-sm text-blue-500 mt-1">{{ stats.totalViews }} total views</div>
          </div>

          <!-- Pengacara Aktif -->
          <div class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl shadow p-6 flex flex-col text-white">
            <div class="flex items-center justify-between mb-2">
        ...
        <div class="mb-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="article in recentArticles.slice(0, 3)" :key="article.artikel_id"
              class="bg-white rounded-xl shadow p-4 hover:shadow-lg transition-shadow"
            >
              <div class="mb-3">
                <img
                  :src="article.gambar || '/src/assets/images/default-article.jpg'"
                  :alt="article.judul"
              ...
              <div>
                <h3 class="font-semibold text-sm mb-2 line-clamp-2">{{ article.judul }}</h3>
                <p class="text-xs text-gray-500 mb-2 line-clamp-2">{{ article.deskripsi }}</p>
            ...
            <div v-if="recentArticles.length === 0" class="col-span-full">
              <div class="bg-white rounded-xl shadow p-8 text-center">
                <span class="material-icons text-gray-300 text-4xl mb-2">article</span>
        ...
        <div class="bg-white rounded-xl shadow p-6">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
              ...
              <tbody>
                <tr v-for="booking in recentBookings.slice(0, 3)" :key="booking.booking_id">
                  <td class="py-2 px-4 flex items-center">
                    <img
                      class="w-8 h-8 rounded-full mr-2"
                      :src="`https://randomuser.me/api/portraits/${booking.booking_id % 2 === 0 ? 'men' : 'women'}/${40 + booking.booking_id}.jpg`"
                    />
                    <div>
                      <div class="font-semibold">{{ booking.nama_pembooking }}</div>
                  ...
                  <td class="py-2 px-4">
                    ...
                    <div class="text-xs text-gray-500">{{ booking.waktu_booking }} WIB</div>
                  </td>
...
const loadStats = async () => {
  try {
    ...
    // Load bookings from API (akan dibuat nanti)
    await loadRecentBookings();
    
    // Mock data untuk statistik lainnya
    stats.value = {
      totalUsers: 142,        // Total klien
      totalArticles: recentArticles.value.length || 28,      // Artikel hukum dari API
      pendingBookings: recentBookings.value.filter(b => b.status === 'pending').length || 7,     // Booking pending approval
      totalLawyers: 8,        // Pengacara aktif
      totalViews: 3250        // Total views artikel
    };
  } catch (error) {
...
const loadRecentArticles = async () => {
  try {
    const response = await fetch('http://localhost:3000/api/article/get-articles?limit=5&offset=0');
...
const loadRecentBookings = async () => {
  try {
    ...
    if (token) {
      // Try to call the API
      const response = await fetch('http://localhost:3000/api/booking/get-bookings?limit=5&offset=0', {
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success && data.bookings) {
          ...
          console.log('Recent bookings loaded from API:', recentBookings.value);
          return;
        }
      }
    }
    ...
    recentBookings.value = [
      ...
      {
        booking_id: 2,
        nama_pembooking: 'Budi Santoso',
        email: 'budi.santoso@email.com',
        nomor_handphone: '081234567891',
        tanggal_booking: '2025-07-13',
        waktu_booking: '10:00',
        status: 'approved',
        lawyer_name: 'Sarah Dewi',
        specialty: 'Hukum Bisnis',
        created_at: '2025-07-10T08:00:00Z'
      },
      {
        booking_id: 3,
        nama_pembooking: 'Maya Indri',
        email: 'maya.indri@email.com',
        nomor_handphone: '081234567892',
        tanggal_booking: '2025-07-12',
        waktu_booking: '16:00',
        status: 'rejected',
        lawyer_name: 'Ahmad Rizki',
...
const approveBooking = (bookingId) => {
  alert(`Booking ID ${bookingId} disetujui! Klien akan mendapat notifikasi via email.`);
  // TODO: Implement API call to approve booking
};

const rejectBooking = (bookingId) => {
  alert(`Booking ID ${bookingId} ditolak! Klien akan mendapat notifikasi untuk memilih waktu lain.`);
