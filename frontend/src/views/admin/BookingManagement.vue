    ...
    >
      ...
      <nav class="mt-6">
        <div class="px-4 space-y-2">
          ...
          <button
            @click="$router.push('/admin/lawyer-management')"
            class="w-full flex items-center px-4 py-3 text-left text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg transition-colors duration-200"
          >
            <svg
              class="w-5 h-5 mr-3"
    ...
    <div class="flex-1 flex flex-col">
      ...
      <main class="flex-1 overflow-y-auto p-6">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-8">
          ...
          <!-- Sorting info -->
          <div v-if="sortField" class="mt-4 p-3 bg-blue-50 rounded-lg">
            <div class="flex items-center space-x-2">
              ...
              <span class="text-sm text-blue-600">
                ...
                <strong>{{ getSortFieldName(sortField) }}</strong> ({{
                  sortDirection === "asc"
                    ? "A-Z / Terkecil ke Terbesar"
                    : "Z-A / Terbesar ke Terkecil"
                }})
              </span>
              <button
                @click="clearSort"
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
          ...
          <!-- Empty state -->
          <div v-else-if="sortedBookings.length === 0" class="p-8 text-center">
            <svg
          ...
          <div v-else class="overflow-x-auto">
            <table class="w-full">
              <thead class="bg-gray-50">
                <tr>
                  <th
                    @click="sortBy('booking_id')"
                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                  >
                    <div class="flex items-center space-x-1">
                      <span>ID</span>
                      <svg
                        v-if="sortField === 'booking_id'"
                        class="w-4 h-4"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                  ...
                  <th
                    @click="sortBy('nama_pembooking')"
                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                  >
                    <div class="flex items-center space-x-1">
                      <span>Klien</span>
                      <svg
                        v-if="sortField === 'nama_pembooking'"
                        class="w-4 h-4"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                  ...
                  <th
                    @click="sortBy('lawyer_name')"
                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                  >
                    <div class="flex items-center space-x-1">
                      <span>Lawyer</span>
                      <svg
                        v-if="sortField === 'lawyer_name'"
                        class="w-4 h-4"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                  ...
                  <th
                    @click="sortBy('tanggal_booking')"
                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                  >
                    <div class="flex items-center space-x-1">
                      <span>Tanggal & Waktu</span>
                      <svg
                        v-if="sortField === 'tanggal_booking'"
                        class="w-4 h-4"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
      ...
      >
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-medium">
            Detail Booking #{{ selectedBooking.booking_id }}
          </h3>
          <button
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <p class="mt-1 text-sm text-gray-900">
              {{ selectedBooking.pesan || "Tidak ada pesan" }}
            </p>
          </div>
...
const sortedBookings = computed(() => {
  ...
  return [...bookings.value].sort((a, b) => {
    ...
    if (sortField.value === "booking_id") {
      aValue = parseInt(aValue) || 0;
      bValue = parseInt(bValue) || 0;
    } else if (sortField.value === "tanggal_booking") {
...
const loadBookings = async () => {
  ...
  try {
    ...
    const response = await fetch(
      `https://mptibe-production.up.railway.app/api/booking/get-bookings?${params}`,
      {
        headers: {
...
const approveBooking = async (bookingId) => {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(
      `https://mptibe-production.up.railway.app/api/booking/update-booking`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
...
const getSortFieldName = (field) => {
  const fieldNames = {
    ...
    nama_pembooking: "Nama Klien",
    lawyer_name: "Nama Lawyer",
